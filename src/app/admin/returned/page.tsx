...
export default function ReturnedOrdersPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const filtered = allData.filter(o => o.deliveryStatus === 'returned')
      setOrders(filtered)
      setLoading(false)
    }, (error) => {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
        <div>
          <h1 className="text-3xl font-black text-[var(--heading-color)] dark:text-white flex items-center gap-3">
            ...
            {t('admin.dashboard_returned_orders') || 'الطلبات المرتجعة'}
          </h1>
          <p className="text-slate-500 mt-1 font-bold">{t('admin.returned_orders_desc') || 'عرض الطلبات التي تم إرجاعها (قراءة فقط)'}</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 max-h-[800px] overflow-y-auto pr-4 custom-scrollbar">
        {filteredOrders.length === 0 ? (
          ...
          filteredOrders.map((order) => (
            <div key={order.id} className="bg-white dark:bg-slate-800/50 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-xl p-8 opacity-90 hover:opacity-100 transition-all hover:border-red-400/50">
              <div className="flex flex-wrap justify-between items-start gap-6">
                <div className="flex items-center gap-6">
                  <div className="w-14 h-14 rounded-2xl bg-red-500 flex items-center justify-center font-black text-xl text-white shadow-lg shadow-red-500/20">
                    <Undo2 size={28} />
                  </div>
                  <div>
                ...
                <div className="flex flex-col items-end">
                  ...
                  <div className="text-xl font-black text-red-500">{order.totalAmount.toLocaleString('ar-EG')} {t('common.currency')}</div>
                </div>
              </div>
