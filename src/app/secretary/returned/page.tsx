...
export default function ReturnedOrdersSecretaryPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // Filter: my orders that have been returned
      const myReturnedOrders = allOrders.filter(o => {
        const isMine = (secretary.id && o.secretaryId === secretary.id) || 
                       (secretary.email && o.secretaryEmail === secretary.email)
        return isMine && (o.deliveryStatus === 'returned')
      })

      const sortedData = [...myReturnedOrders].sort((a, b) => {
      ...
      console.error("Returned orders listener error:", error)
      setLoading(false)
    })
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-3xl font-black text-[var(--heading-color)] dark:text-white flex items-center gap-3">
            <Undo2 className="text-red-500" size={32} />
            {t('sec_returned_title')}
          </h1>
          <p className="text-slate-500 mt-1 font-bold">{t('sec_returned_subtitle')}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="bg-red-100 dark:bg-red-500/10 text-red-600 dark:text-red-400 px-6 py-3 rounded-2xl font-black shadow-sm">
      ...
      <div className="grid grid-cols-1 gap-6 max-h-[700px] overflow-y-auto pr-4 custom-scrollbar">
        {filteredOrders.length === 0 ? (
          <div className="bg-white dark:bg-slate-800/50 p-20 rounded-[3rem] text-center border-2 border-dashed border-slate-200 dark:border-slate-700">
            ...
            <p className="text-xl font-black text-slate-400">{t('sec_returned_empty')}</p>
          </div>
        ) : (
          filteredOrders.map((order) => (
            <div key={order.id} className="bg-white dark:bg-slate-800/50 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-xl p-6 md:p-8 hover:border-red-300 dark:hover:border-red-500/30 transition-all">
              <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                <div className="flex gap-4 md:gap-6 w-full md:w-auto">
                  <div className="w-12 h-12 md:w-14 md:h-14 shrink-0 rounded-2xl bg-red-500 flex items-center justify-center font-black text-xl text-white shadow-lg shadow-red-500/20">
                    <Undo2 size={24} className="md:w-7 md:h-7" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <div className={`px-3 py-1.5 rounded-xl border text-xs font-black w-fit bg-red-50 dark:bg-red-500/10 text-red-500 border-red-100 dark:border-red-500/20`}>
                          {t('order.status_returned')}
                        </div>
                      </div>
                    </div>
                ...
                <div className="flex flex-row md:flex-col justify-between items-center md:items-end w-full md:w-auto pt-4 md:pt-0 border-t md:border-t-0 border-slate-100 dark:border-slate-700">
                  ...
                  <div className="text-xl font-black text-red-500">{order.totalAmount?.toLocaleString(dir === 'rtl' ? 'ar-EG' : 'en-US')} {t('common.currency')}</div>
                </div>
              </div>
